(function($) {
  ...
  $.fn.fullClip = function(options) {
    ...
    for (i = 0, end = settings.images.length; i < end; ++i) {
        ...
        $(".caption").html(settings.text[i]);
        $(".container h5").html(settings.subtitle[i]);
    }
    ...
    (function update() {
      settings.current = (settings.current + 1) % settings.images.length;
        ...
        $(".caption").html(settings.text[settings.current])
        $(".container h5").html(settings.subtitle[settings.current])
    }());
}}(jQuery));
